...
fun DuenosScreen(viewModel: MainViewModel, navController: NavController) {
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                Text("Gestión de Dueños", style = MaterialTheme.typography.headlineMedium, modifier = Modifier.padding(bottom = 16.dp))
            }
            items(duenos) { dueno ->
                DuenoCard(
                    dueno = dueno,
    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(dueno.nombre, style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                Text("ID: ${dueno.id}")
                Text(dueno.email)
                Text(dueno.telefono)
            }
